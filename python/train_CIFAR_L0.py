...
def get_args():
    ...
    # Sparsity
    parser.add_argument('--L0_threshold', type=float, default=1e-3, help="Threshold for L0 regularization")
    parser.add_argument('--pruning_steps', type=int, default=10, help="number of pruning steps")
    parser.add_argument('--start_pruning', type=int, default=10, help="epoch to start pruning")
...
class L0PruningModule(torch.nn.Module):
    def __init__(self, initial_threshold=0):
        super(L0PruningModule, self).__init__()
        self.threshold = initial_threshold

    def forward(self, x):
        # Apply L0 pruning
        pruned_weights = torch.where(torch.abs(x) < self.threshold, torch.zeros_like(x), x)
...
def main():
    ...
    # Pruning
    pruning_module = L0PruningModule()
    def threshold_epoch(epoch):
        # linear growth from 0 to L0_threshold over pruning_steps epochs, starting at start_pruning
        if epoch < args.start_pruning:
            return 0
        else:
            return min(args.L0_threshold, args.L0_threshold * (epoch - args.start_pruning) / args.pruning_steps)
        ...
        if epoch >= args.start_pruning:
            ...
            with torch.no_grad():
                for param in model.parameters():
                    param.copy_(pruning_module(param))
